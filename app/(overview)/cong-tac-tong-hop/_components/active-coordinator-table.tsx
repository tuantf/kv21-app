...
export const ActiveCoordinatorTable = ({
  data,
  isLoading,
}: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await db.transact(
        db.tx.tonghop[editingRowId].update({
          name: sanitizedName,
          link: sanitizedLink,
          supervisor: sanitizedSupervisor,
          updated: now,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await db.transact(db.tx.tonghop[deletingRowId].delete())
  ...
  const handleMarkAsCompleted = async (row: TonghopRow) => {
    try {
      const now = new Date().toISOString()
      ...
      // Delete from tonghop
      await db.transact(db.tx.tonghop[row.id].delete())
    } catch (error) {
