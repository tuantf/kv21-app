...
export const ActiveTopicTable = ({
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await db.transact(
        db.tx.chuyende[editingRowId].update({
          name: sanitizedName,
          doc: sanitizedDoc,
          link: sanitizedLink,
          report: sanitizedReport,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await db.transact(db.tx.chuyende[deletingRowId].delete())
  ...
  const handleMarkAsCompleted = async (row: ChuyendeRow) => {
    try {
      const now = new Date().toISOString()
      ...
      await db.transact(db.tx.chuyende[row.id].delete())
    } catch (error) {
